...
import { paginationTours } from "../../helpers/pagination.helper";
...
export const index = async (req: Request, res: Response) => {
  try {
    ...
    // Hết sắp xếp
    //Phân trang
    const pagination = await paginationTours(req, find);
    //Hết  Phân trang
...
export const createPost = async (req: Request, res: Response) => {
  if (res.locals.role.permissions.includes("tours_create")) {
...
export const editPatch = async (req: Request, res: Response) => {
  if (res.locals.role.permissions.includes("tours_edit")) {
...
export const deleteTour = async (req: Request, res: Response) => {
  if (res.locals.role.permissions.includes("tours_delete")) {
    try {
      const id = req.params.id;
      await TourCategory.destroy({
        where: {
